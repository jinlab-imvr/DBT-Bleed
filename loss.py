...
class LossSigmoid(nn.Module):
    ...
    def forward(self, logits, labells, dec):
        ...
        if torch.isnan(logits).any() or torch.isinf(logits).any():
            print(f"[ERROR] NaN/Inf in logits! min={logits.min().item():.4f} max={logits.max().item():.4f}")
        ...
        if torch.isnan(loss_each).any():
            print(f"[ERROR] NaN in loss_each after log_sigmoid! input_sig range: [{input_sig.min().item():.4f}, {input_sig.max().item():.4f}]")
        ...
        if torch.isnan(loss).any():
            print(f"[ERROR] NaN in final loss! dec_each_in: {dec_each_in}")
